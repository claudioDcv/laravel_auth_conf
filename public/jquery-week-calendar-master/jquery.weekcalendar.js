 ...
 * jQuery.weekCalendar v3.0-dev
 * - Remove suppor jquery 1.x.x
 * - require
 *  - jquery 2.0.0
      ...
      _renderCalendarButtons: function($calendarContainer) {
        ...
        if (options.buttons) {
            ...
            calendarNavHtml += '<div class=\"wc-toolbar\">';
              calendarNavHtml += '<div class=\"wc-display\"></div>';
              calendarNavHtml += '<div class=\"mwc-nav btn-group\">';
                calendarNavHtml += '<button class=\"mwc-prev btn btn-default\">' + options.buttonText.lastWeek + '</button>';
                calendarNavHtml += '<button class=\"mwc-today btn btn-default\">' + options.buttonText.today + '</button>';
                calendarNavHtml += '<button class=\"mwc-next btn btn-default\">' + options.buttonText.nextWeek + '</button>';
              calendarNavHtml += '</div>';
              calendarNavHtml += '<h1 class=\"wc-title\"></h1>';
            calendarNavHtml += '</div>';
            ...
            $calendarContainer.find('.mwc-nav .mwc-today')
              // .button({
              //   icons: {primary: 'ui-icon-home'}})
              .on('click',function() {
                    self.today();
            ...
            $calendarContainer.find('.mwc-nav .mwc-prev')
              // .button({
              //   text: false,
              //   icons: {primary: 'ui-icon-seek-prev'}})
              .on('click',function() {
            ...
            $calendarContainer.find('.mwc-nav .mwc-next')
              // .button({
              //   text: false,
              //   icons: {primary: 'ui-icon-seek-next'}})
              .on('click',function() {
            ...
            if (this.options.switchDisplay && $.isPlainObject(this.options.switchDisplay)) {
              ...
              $.each(this.options.switchDisplay, function(label, option) {
                      ...
                      var _input = $('<input type="radio" id="' + _id + '" name="wc-switch-display" class="cwc-switch-display"/>');
                      var _label = $('<label for="' + _id + '"></label>');
                      _label.html('<div class="btn btn-primary">'+label+'</div>');
                      _input.val(option);
